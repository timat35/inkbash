...
canreg_ageSpecific_rate_multi_plot <- function(dt, 
  ...
  if (return_data) {
    dt[, rate := get(var_cases)/get(py)*10000]
    dt[, cancer_sex := NULL]
    ...
    dt <- dt[, c("cancer_label",
                 ...
                 var_py,
                 "rate"), with=FALSE]
    setkeyv(dt, c("ICD10GROUP", group_by,var_age))
    return(dt)
    stop() 
  }
...
if (return_data) {
	...
	setnames(dt_data, var_age_label_list,"AGE_GROUP_LABEL")
	print(colnames(dt_data))
	
	if ("ICD10GROUP" %in% colnames(dt_data)) {
	  
	  temp_col <- c(var_top,"ICD10GROUP","cancer_rank",group_by,var_age,"AGE_GROUP_LABEL",var_cases,var_py,"rate")
	  
	} else {
	  temp_col <- c(var_top,"cancer_rank",group_by,var_age,"AGE_GROUP_LABEL",var_cases,var_py,"rate")
	}
	
	dt_data[, rate := get(var_cases)/get(var_py)*10000]
    ...
    dt_data <- dt_data[, temp_col, with=FALSE]
    setkeyv(dt_data, c(group_by,"cancer_rank",var_top ,var_age ))
    return(dt_data)
    stop() 
}
...
canreg_bar_top <- function(df_data,
  ...
  dt[CSU_BY==levels(dt$CSU_BY)[[2]], asr_plot:= CSU_ASR*1]
  ...
  dt[asr_round %in% c(" 0.0","  0.0", "0.0"), asr_round := ""]
...
canreg_population_pyramid <- function(df_data,
  ...
  setnames(dt, var_bar, "CSU_BAR")
  setnames(dt, var_age_cut, "CSU_AGE_GROUP")
  setnames(dt, group_by, "CSU_BY")
  ...
  factor_order <- unique(dt[, c("CSU_BAR", "CSU_AGE_GROUP"), with=FALSE])
  dt$CSU_BAR <- factor(dt$CSU_BAR,
                       levels = setkeyv(factor_order, "CSU_AGE_GROUP")$CSU_BAR) 
  ...
  dt_poly1 <- dt[CSU_AGE_GROUP == max(CSU_AGE_GROUP),.(CSU_AGE_GROUP, cases_plot, CSU_BY)] 
  dt_poly2 <- dt_poly1
  dt_poly2$cases_plot <- 0
  dt_poly3 <- dt_poly2
  dt_poly3$CSU_AGE_GROUP =  dt_poly3$CSU_AGE_GROUP+1
  dt_poly <- rbind(dt_poly1,dt_poly2,dt_poly3)
  dt_poly[,CSU_AGE_GROUP :=CSU_AGE_GROUP+0.5]
  dt_poly[CSU_AGE_GROUP == max(CSU_AGE_GROUP),CSU_AGE_GROUP :=CSU_AGE_GROUP+1]
  
  dt[CSU_AGE_GROUP == max(CSU_AGE_GROUP), cases_plot :=0 ]
  ...
  csu_plot <- ggplot(dt, aes(CSU_BAR, cases_plot, fill=CSU_BY)) +
    ...
    geom_polygon(data=dt_poly, aes(x=CSU_AGE_GROUP, y=cases_plot, group=CSU_BY), colour="black" )+
    geom_hline(yintercept = 0, colour="black",size = line_size)+
    coord_flip(ylim = c(tick_minor_list[1]-(tick_space*0.25),tick_minor_list[length(tick_minor_list)]+(tick_space*0.25)), expand = TRUE)+
...
canreg_cases_age_bar <- function(df_data,
  ...
  dt[CSU_BY==levels(dt$CSU_BY)[[1]], cases_plot:= CSU_CASES*(-1)]
  dt[CSU_BY==levels(dt$CSU_BY)[[2]], cases_plot:= CSU_CASES*1]
